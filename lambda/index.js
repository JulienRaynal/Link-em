...
const AskGoogleIntent = {
    canHandle(handlerInput){
        return Alexa.getRequestType(handlerInput.requestEnvelope) === 'AskGoogleIntent';
    },
    handle(handlerInput){
        
 const etat = Alexa.getSlotValue(handlerInput.requestEnvelope, 'etat')
 const appareil = Alexa.getSlotValue(handlerInput.requestEnvelope, 'object')
let  object = {
     "name" : appareil,
     "state" : etat.id
 }
 var options = {
            'method': "POST",
            'url': "http://86.227.145.205/api/alexa",
            'headers': {
